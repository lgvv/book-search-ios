...
struct DefaultRecentlyViewedRepositoryTests {
    ...
    func 책정보를_함께보관한다() async throws {
        ...
        let items = try await self.sut.list()
        #expect(items.first?.book == book)
    }

    @Test
    func 기록한적없으면_빈목록이다() async throws {
        let items = try await self.sut.list()
    ...
    @Test
    func 최근에본책이_목록맨위에온다() async throws {
        try await self.sut.record(book: pachinko, keeping: 20)
        try await self.sut.record(book: toji, keeping: 20)
        try await self.sut.record(book: humanActs, keeping: 20)

        let items = try await self.sut.list()
        #expect(items.map(\.book.isbn) == [humanActs.isbn, toji.isbn, pachinko.isbn])
    }

    @Test
    func 같은책을다시보면_중복항목이생기지않는다() async throws {
        try await self.sut.record(book: pachinko, keeping: 20)
    ...
    @Test
    func 같은책을다시보면_맨위로올라온다() async throws {
        try await self.sut.record(book: pachinko, keeping: 20)
        try await self.sut.record(book: toji, keeping: 20)
        ...
        let items = try await self.sut.list()
        #expect(items.map(\.book.isbn) == [pachinko.isbn, toji.isbn])
    }

    @Test
    func 같은책을다시보면_본시각이갱신된다() async throws {
        try await self.sut.record(book: pachinko, keeping: 20)
        let firstList = try await self.sut.list()
        let firstViewedAt = try #require(firstList.first?.viewedAt)

        try await self.sut.record(book: pachinko, keeping: 20)

        let laterList = try await self.sut.list()
        let laterViewedAt = try #require(laterList.first?.viewedAt)
        #expect(laterViewedAt > firstViewedAt)
    }

    @Test
    func 같은책을다시보면_바뀐책정보로갱신된다() async throws {
        ...
        let items = try await self.sut.list()
        #expect(items.first?.book.title == "새 제목")
    }

    @Test
    func 상한을넘으면_가장오래본책부터밀려난다() async throws {
        for index in 1 ... 5 {
            try await self.sut.record(book: Book(isbn: "\(index)", title: "책\(index)"), keeping: 3)
        }

        let items = try await self.sut.list()
        #expect(items.map(\.book.isbn) == ["5", "4", "3"])
    }

    @Test
    func 상한안에서는_아무것도밀려나지않는다() async throws {
        for index in 1 ... 3 {
    ...
    @Test
    func 상한초과여부는_재열람으로항목수가늘지않을때는영향이없다() async throws {
        for index in 1 ... 3 {
            try await self.sut.record(book: Book(isbn: "\(index)", title: "책\(index)"), keeping: 3)
        }
        ...
        let items = try await self.sut.list()
        #expect(items.map(\.book.isbn) == ["1", "3", "2"])
    }

    @Test
    func 한항목을삭제하면_그책만빠진다() async throws {
        try await self.sut.record(book: pachinko, keeping: 20)
        try await self.sut.record(book: toji, keeping: 20)
        ...
        let items = try await self.sut.list()
        #expect(items.map(\.book.isbn) == [toji.isbn])
    }

    @Test
    func 없는항목을삭제해도_실패하지않는다() async throws {
        try await self.sut.remove(isbn: "없는isbn")
    }

    @Test
    func 전체를지우면_목록이비워진다() async throws {
        try await self.sut.record(book: pachinko, keeping: 20)
        try await self.sut.record(book: toji, keeping: 20)
    ...
    @Test
    func 비어있는목록을지워도_실패하지않는다() async throws {
        try await self.sut.clear()
    }

    @Test
    func 전체를지운뒤에도_다시기록할수있다() async throws {
        try await self.sut.record(book: pachinko, keeping: 20)
        try await self.sut.clear()
