...
struct DiskCacheTests {
    ...
    @Test
    func 저장한본문을_같은URL로다시읽을수있다() async {
    ...
    @Test
    func URL이다르면_서로다른항목으로저장된다() async {
        let sut = self.makeSUT()
        ...
        #expect(first?.etag == "a")
        #expect(second?.etag == "b")
    }

    @Test
    func 같은키에다시저장하면_세대가올라간다() async {
        let sut = self.makeSUT()
        ...
        #expect(second?.etag == "v2")
    }

    @Test
    func 새세대를커밋하면_이전세대파일을지운다() async {
        let sut = self.makeSUT()
        await sut.store(self.makeBody(10), etag: "v1", for: self.coverURL)
    ...
    @Test
    func 축출된키를곧바로다시저장해도_세대번호가되돌아가지않는다() async {
        ...
        #expect(wasEvicted == nil, "축출되지 않으면 이 테스트의 전제가 성립하지 않는다")

        await sut.store(self.makeBody(100), etag: "a2", for: self.coverURL)

        let reborn = await sut.entry(for: self.coverURL)
        #expect(reborn?.etag == "a2")
        #expect((reborn?.generation ?? 0) > (firstGeneration ?? 0))
    }

    @Test
    func 지난실행이남긴세대파일보다_새세대번호가항상크다() async {
        ...
        let entry = await sut.entry(for: self.coverURL)
        #expect((entry?.generation ?? 0) > 99)
    }

    @Test
    func freshLifetime안에저장한항목은_신선하다() async {
    ...
    @Test
    func freshLifetime이지난항목은_값은주되신선하지않다() async {
    ...
    @Test
    func 항목크기상한을넘는본문은_저장하지않는다() async {
    ...
    @Test
    func 항목크기상한을넘는저장은_기존항목을건드리지않는다() async {
        let sut = self.makeSUT(maxByteSize: 1_000, maxEntryByteSize: 100)
        ...
        #expect(survivor?.etag == "작은것")
    }

    @Test
    func 총량상한을넘으면_가장오래안쓴항목부터지운다() async {
        ...
        #expect(survivor?.etag == "a")
        #expect(evicted == nil)
        #expect(newest?.etag == "c")
    }

    @Test
    func 방금저장한항목은_축출대상에서제외한다() async {
        ...
        #expect(newest?.etag == "b")
    }

    @Test
    func 본문쓰기가실패하면_색인에올리지않는다() async {
        let sut = self.makeSUT()
    ...
    @Test
    func 색인에는있는데파일이없으면_nil을주고색인을정리한다() async {
        let sut = self.makeSUT()
        await sut.store(self.makeBody(10), etag: "v1", for: self.coverURL)
        ...
        await sut.store(self.makeBody(20), etag: "v2", for: self.coverURL)
        let reborn = await sut.entry(for: self.coverURL)
        #expect(reborn?.etag == "v2")
    }

    @Test
    func 색인에없는본문파일은_시작할때청소한다() async {
        ...
        #expect(self.files.keys.subtracting(["manifest.json"]) == [])
    }

    @Test
    func 매니페스트형식이다르면_색인을통째로버린다() async {
        ...
        #expect(!(self.files.keys.contains("\(key).1")))
    }

    @Test
    func 매니페스트가있으면_다음실행에서도항목을찾는다() async {
