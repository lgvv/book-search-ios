...
struct DefaultRecentSearchRepositoryTests {
    ...
    @Test
    func 검색어를기록하면_목록에서읽을수있다() async throws {
    ...
    @Test
    func 새검색어는_목록맨앞에놓인다() async throws {
    ...
    @Test
    func 이미있는검색어를다시기록하면_중복없이맨앞으로올라온다() async throws {
        try await self.sut.record(term: "민음사", keeping: 10)
        ...
        let terms = try await self.sut.list()
        #expect(terms == ["민음사", "토지", "파친코"])
    }

    @Test
    func 상한을넘으면_가장오래된검색어부터밀려난다() async throws {
        for index in 1 ... 5 {
            try await self.sut.record(term: "검색어\(index)", keeping: 3)
        }

        let terms = try await self.sut.list()
        #expect(terms == ["검색어5", "검색어4", "검색어3"])
    }

    @Test
    func 상한이1이면_가장최근검색어하나만남는다() async throws {
    ...
    @Test
    func 상한이줄어들면_다음기록에서목록이잘린다() async throws {
        for index in 1 ... 5 {
        ...
        let terms = try await self.sut.list()
        #expect(terms == ["새검색어", "검색어5"])
    }

    @Test
    func 검색어를제거하면_목록에서빠진다() async throws {
        try await self.sut.record(term: "민음사", keeping: 10)
        try await self.sut.record(term: "파친코", keeping: 10)
    ...
    @Test
    func 없는검색어를제거해도_목록이그대로다() async throws {
        try await self.sut.record(term: "민음사", keeping: 10)
    ...
    @Test
    func 제거해도_나머지순서는유지된다() async throws {
    ...
    @Test
    func 같은저장소를다시열면_기록이남아있다() async throws {
        try await self.sut.record(term: "민음사", keeping: 10)
