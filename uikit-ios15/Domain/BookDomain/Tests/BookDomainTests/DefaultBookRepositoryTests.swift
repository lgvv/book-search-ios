...
final class DefaultBookRepositoryTests: XCTestCase {
    ...
    func test_검색하면_질의와페이지와크기를질의문자열에싣는다() async throws {
        ...
        XCTAssertEqual(components.path, "/v1/books")
        XCTAssertEqual(components.queryItems?.first { $0.name == "query" }?.value, "민음사")
        XCTAssertEqual(components.queryItems?.first { $0.name == "page" }?.value, "2")
        XCTAssertEqual(components.queryItems?.first { $0.name == "size" }?.value, "30")
    }

    func test_한글질의도_퍼센트인코딩되어나간다() async throws {
        let (sut, transport) = self.makeSUT(
        ...
        let request = try XCTUnwrap(transport.requests.value.first)
        XCTAssertTrue(request.url.absoluteString.contains("%ED%86%A0%EC%A7%80"))
    }

    func test_단건조회하면_isbn을경로에붙인다() async throws {
        ...
        XCTAssertEqual(
            ...
            "https://api.booksearch.dev/v1/books/9788937473135"
        )
    }

    func test_받은건수가전체보다적으면_다음페이지가있다() async throws {
    ...
    func test_현재페이지까지의누적이전체와같으면_다음페이지가없다() async throws {
        let (sut, _) = self.makeSUT(
            handler: self.makeSearchResponse(
                items: [(isbn: "1", title: "책1")],
                page: 5,
    ...
    func test_누적이전체를넘으면_다음페이지가없다() async throws {
        let (sut, _) = self.makeSUT(
            handler: self.makeSearchResponse(
                items: [(isbn: "1", title: "책1")],
                page: 6,
    ...
    func test_결과가하나도없으면_빈페이지를돌려준다() async throws {
        let (sut, _) = self.makeSUT(
            handler: self.makeSearchResponse(items: [], page: 1, pageSize: 20, totalCount: 0)
        )
        ...
        XCTAssertEqual(page.books, [])
        XCTAssertEqual(page.totalCount, 0)
        XCTAssertFalse(page.hasNext)
    }

    func test_응답항목을_도메인모델로옮긴다() async throws {
        let body = Data("""
    ...
    func test_표지URL이빈문자열이면_nil로옮긴다() async throws {
        ...
        XCTAssertNil(page.books.first?.coverImageURL)
    }

    func test_없는책을조회하면_404를nil로본다() async throws {
        let (sut, _) = self.makeSUT { request in
    ...
    func test_단건조회가404외의오류로실패하면_던진다() async {
        let (sut, _) = self.makeSUT { request in
